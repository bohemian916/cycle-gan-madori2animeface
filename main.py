...
A_DIR_VAL = './datasets/'+TARGET+'/testA/*'
B_DIR_VAL = './datasets/'+TARGET+'/testB/*'

RESULT_DIR = './results/'+TARGET+'/img/'
...
fa, a_val = dataset.get_image_batch(A_DIR_VAL,VAL_BATCH_SIZE,256,256, train=False)
fb, b_val = dataset.get_image_batch(B_DIR_VAL,VAL_BATCH_SIZE,256,256, train=False)

with tf.variable_scope('gen_a_to_b') as a_to_b_scope :
    b_gen = build_enc_dec(a)
    a_to_b_scope.reuse_variables()
    b_gen_val = build_enc_dec(a_val, reuse=True)
with tf.variable_scope('gen_b_to_a') as b_to_a_scope :
    a_gen = build_enc_dec(b)
    b_to_a_scope.reuse_variables()
    a_gen_val = build_enc_dec(b_val, reuse=True)

with tf.variable_scope('gen_b_to_a',reuse=True) as gen_b_to_a_scope:
    a_identity = build_enc_dec(b_gen,True)
    gen_b_to_a_scope.reuse_variables()
    a_identity_val = build_enc_dec(b_gen_val,True)
with tf.variable_scope('gen_a_to_b',reuse=True) as gen_a_to_b_scope:
    b_identity = build_enc_dec(a_gen,True)
    gen_a_to_b_scope.reuse_variables()
    b_identity_val = build_enc_dec(a_gen_val,True)
...
def _save_to_file(n,a,b,a_b, b_a) :
    a_i = [_convert_to_image(i) for i in a]
    b_i = [_convert_to_image(i) for i in b]
    a_b_i = [_convert_to_image(i) for i in a_b]
    b_a_i = [_convert_to_image(i) for i in b_a]

    image_array = [a_i, a_b_i, b_i, b_a_i]
    width, height = a_i[0].size 

    line_imgs = []
    for images in image_array:
        line_im = Image.new('RGB', (width*10, height))

        x_offset = 0
        for im in images:
            line_im.paste(im, (x_offset,0))
            x_offset += width 
        line_imgs.append(line_im) 

    res_img = Image.new('RGB', (width*10, height*4))

    y_offset = 0
    for im in line_imgs:
        res_img.paste(im, (0, y_offset))
        y_offset += height 

    f = os.path.join(RESULT_DIR,"img"+ str(n)+'.jpg')
    print(f)
    res_img.save(f)
...
try:
    ...
    for step in xrange(MAX_ITERATION+1) :
        ...
        if( step % SAVE_PERIOD == 0 ):
            ...
            b_gen_vals =[]
            for _ in xrange(10):
                a_eval,b_gen_eval = sess.run([a_val,b_gen_val])
                a_vals.append(a_eval)
                b_gen_vals.append(b_gen_eval)

                b_eval,a_gen_eval = sess.run([b_val,a_gen_val])
                b_vals.append(b_eval)
                a_gen_vals.append(a_gen_eval)

            _save_to_file(int(step/SAVE_PERIOD),a_vals, b_vals, b_gen_vals, a_gen_vals)
